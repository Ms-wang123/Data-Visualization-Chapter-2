...
// 图表类型定义 - 基于第2章内容
const chartTypes = [
    { id: 'line', name: '折线图', title: '未来15天最高气温和最低气温', description: '2.1.1 使用plot()绘制折线图展示温度变化趋势' },
    { id: 'bar', name: '柱形图', title: '2013—2019财年阿里巴巴淘宝和天猫平台的GMV', description: '2.2.2 使用bar()绘制柱形图展示财务数据' },
    { id: 'horizontalBar', name: '条形图', title: '各商品种类的网购替代率', description: '2.3.2 使用barh()绘制条形图展示替代率数据' },
    { id: 'area', name: '堆积面积图', title: '物流公司物流费用统计', description: '2.4.2 使用stackplot()绘制堆积面积图展示费用变化' },
    { id: 'histogram', name: '直方图', title: '人脸识别的灰度直方图', description: '2.5.2 使用hist()绘制直方图展示数据分布' },
    { id: 'pie', name: '饼图', title: '支付宝月账单报告', description: '2.6.2 使用pie()绘制饼图展示消费占比' },
    { id: 'scatter', name: '散点图', title: '汽车速度与制动距离的关系', description: '2.7.2 使用scatter()绘制散点图展示相关性' },
    { id: 'boxplot', name: '箱形图', title: '2017年和2018年全国发电量统计', description: '2.8.2 使用boxplot()绘制箱形图展示数据分布' },
    { id: 'radar', name: '雷达图', title: '霍兰德职业兴趣测试', description: '2.9.2 绘制雷达图展示多维度数据比较' },
    { id: 'errorbar', name: '误差棒图', title: '4个树种不同季节的细根生物量', description: '2.10.2 使用errorbar()绘制误差棒图展示数据的不确定性' }
];
...
const datasets = {
    ...
    horizontalBar: {
        labels: ['家政、家教、保姆等生活服务', '飞机票、火车票', '家具', '手机、手机配件', '计算机及其配套产品', 
                ...
                '家用电器', '食品、饮料、烟酒、保健品', '家庭日杂用品', '保险、演出票务', 
...
createApp({
    setup() {
        ...
        const updateDataTable = () => {
            try {
                ...
                if (chartType === 'scatter') {
                    ...
                    currentDataHeaders.value = ['类别'].concat(dataConfig.datasets.map(d => d.label));
                    currentChartData.value = dataConfig.labels.map((label, index) => {
                        const row = { '类别': label };
        ...
        const renderChart = () => {
            ...
            const chartType = selectedChartType.value;
            const dataConfig = JSON.parse(JSON.stringify(datasets[chartType]));
